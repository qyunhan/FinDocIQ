...
import os, sys, json, io, re, argparse, datetime
...
class GCell(BaseModel):
    value:      str = Field(description="cell value exactly as printed; '' for any empty cell (shaded or not). A shaded cell with vertical borders on both sides is still a normal empty cell — value='' span=1 merge_type='none'.")
    span:       int = Field(default=1, description=(
        "number of leaf columns this cell physically covers. "
        "VERTICAL BORDER RULE: span>1 if and only if there is NO vertical border line between the covered columns. "
        "A value centered across N columns with no dividers → span=N. "
        "A shaded/greyed cell that still has vertical borders on both sides → span=1. "
        "sum(span) across all cells in a row MUST equal len(columns)."
    ))
    merge_type: str = Field(default="none", description=(
        "One of: "
        "'none' = span=1, vertical borders present (normal cell, including empty/shaded ones); "
        "'aggregate' = span>1, a value spanning multiple columns because no vertical border exists between them "
        "(e.g. a number centered across several maturity-bucket columns with no divider); "
        "'blank' = span>1, empty region spanning multiple columns with no vertical border between them; "
        "'repeat' = span>1, same value applies independently to each covered column. "
        "ALWAYS 'none' when span=1. ALWAYS 'aggregate' or 'blank' when span>1."
    ))
...
class GRow(BaseModel):
    ...
    values:   list[GCell] = Field(default_factory=list, description="cell values left-to-right as GCell objects; [] for section_header/sub_header/note rows. sum(cell.span) MUST equal len(columns)")
    ...
    def _coerce_values(cls, v):
        ...
        out = []
        for item in v:
            if isinstance(item, GCell):
                out.append(item)
            elif isinstance(item, str):
                out.append(GCell(value=item, span=1))
            elif isinstance(item, dict):
                out.append(GCell(**item))
            else:
                out.append(item)
        return out
...
- rows: EVERY row, top to bottom, each with:
    ...
    * values   — cell values left to right, exactly one GCell per column in `columns`. A section_header / sub_header / note row with no data uses an empty list [].
...
COLUMN ALIGNMENT — MERGED CELLS:
- `values` must always have EXACTLY one GCell per column. Never omit or shift.
- Blank or shaded cells with vertical borders on both sides: emit GCell(value="", span=1, merge_type="none") for each.
- MERGED CELL: look at the IMAGE for missing vertical border lines. If a value has no vertical borders separating it from adjacent column(s) — the cell boundary is absent and the value sits in an undivided span — that is a merged cell. Use GCell(value="X", span=N, merge_type="aggregate"). The sum of all spans in a row MUST equal len(columns).
- For an empty merged region (shaded band with no internal vertical borders): GCell(value="", span=N, merge_type="blank").
- Do NOT fill values left-to-right when a row is sparse. The absence of a vertical border is the definitive signal that a value is merged."""
...
def page_is_narrative(pdf_path: str, page_1based: int, min_numbers: int = 6) -> bool:
...
def col_boundaries_hint(pdf_path: str, pages: list[int]) -> str:
    """Compact column geometry hint prepended to the prompt.
    Lists column index ranges so Gemini can count spans precisely."""
    boundaries = get_column_boundaries(pdf_path, pages)
    if len(boundaries) < 2:
        return ""
    n_cols = len(boundaries) - 1
    xs = [round(x, 1) for x in boundaries]
    col_ranges = ", ".join(
        f"col{i+1}:x{xs[i]}-{xs[i+1]}" for i in range(n_cols)
    )
    return (
        f"[GEOMETRY: {n_cols} data columns with borders at: {col_ranges}. "
        f"Use these x-coordinates to determine span: count how many consecutive column slots "
        f"a merged cell covers by finding which borders are absent between its left and right edges.]\n"
    )
...
def validate_spans(ext: Extraction) -> list[str]:
    """Check sum(cell.span) == len(columns) for every data row.
    ...
    for t in ext.tables:
        ...
        for row in t.rows:
            if not row.values:
                continue
            total = sum(c.span if isinstance(c, GCell) else 1 for c in row.values)
            if total != n:
                issues.append(
                    f"  span mismatch [{t.title[:40]}] row {row.row_id or repr(row.label[:30])}: "
                    f"sum(spans)={total} != ncols={n}"
                )
    return issues
    ...
    for t in ext.tables:
        for row in t.rows:
            for gcell in row.values:
                v = gcell.value if isinstance(gcell, GCell) else gcell
...
def extract_unit(client, pdf_path: str, unit: dict, force_image: bool, with_thinking: bool,
    ...
    # retry with image if the response looks unreasonable.
    image_first = force_image
    ext, usage = _call(attach_image=image_first)
    image_used = image_first

    # reasonableness fallback: retry once WITH image if the first try looks off
    if not image_used and not _reasonable(ext):
...
def write_table(ws, start_row: int, t: GTable) -> int:
    ...
    for row in t.rows:
        ...
        for gcell in row.values:
            ...
            span = gcell.span if isinstance(gcell, GCell) else 1
            if span > 1:
                ws.merge_cells(start_row=r, start_column=col_cursor,
                               end_row=r, end_column=col_cursor + span - 1)
            col_cursor += span
...
def main():
    ...
    ap.add_argument("--chunk-pages", type=int, default=4, metavar="N",
                    help="max pages per Gemini call for spanning sections (default 4; use 0 to disable chunking)")
    args = ap.parse_args()
    ...
    if args.toc:
        TOC_PATH = args.toc
    elif os.path.exists(f"out/{bank_slug}_toc.json"):
        TOC_PATH = f"out/{bank_slug}_toc.json"
    # else keep default out/step1_toc.json

    out_path = args.out or f"out/{bank_slug}.xlsx"
    ...
    for gnum, gunits in groups.items():
        ...
        for u in gunits:
            ...
            if u["type"] in ("single", "multiple") and page_is_narrative(args.pdf, u["pages"][0]):
    ...
    print(f"   Total: {u['calls']} calls, input={u['prompt']:,} output={u['output']:,} "
          f"thinking={u['thinking']:,} tok, ≈ ${u['cost']:.2f}")

...
| **Test** | OCBC 4Q25 CFO slide 6 — two donut_dual_ring charts: "Operating Profit by Key Businesses" (4 segments) and "Operating Profit by Geography" (5 segments). Single-pass result saved to `DELIVERABLE/test_results/slide_06_singlepass.json`. Multi-pass result saved to `DELIVERABLE/test_results/slide_06_multipass.json`. Ground truth from slide: inner ring = FY24, outer ring = FY25. GWB: FY24=51%, FY25=46%. Consumer/PB: FY24=27%, FY25=26%. Markets: FY24=7%, FY25=11%. Insurance: FY24=15%, FY25=17%. |
| **Results — Key Businesses** | Single-pass: 4/4 segments correctly period-assigned. Multi-pass: 3/4 correct — Consumer/PB period-swapped (outputs FY24=26%, FY25=27% when slide shows FY24=27%, FY25=26%). GWB, Markets, Insurance correct in both. |
| **Results — Geography** | Both approaches: all 5 segments correct. No difference. |
| **Multi-pass structural failures** | Beyond the value error, multi-pass JSON had two additional structural problems: (1) each (series, period) pair was emitted as a separate `element_idx` instead of being grouped — 17 elements for what should be 2. (2) `element_type` was `donut_chart` not `donut_dual_ring`, meaning the contract wasn't applied. These are Pass 2 schema pressure artefacts. |
| **Root cause** | The Step 4 pre-mapping in Pass 1 appears to have introduced a period swap specifically for Consumer/PB — or Pass 2 re-interpreted the pre-map when transcribing under schema pressure without the image present. Either way, single-pass avoids this because visual reasoning and schema filling happen in one step with the image present throughout. There is no intermediate text representation that can drift. |
...
| **Slide title issue** | Both approaches failed to extract the slide-level title ("Earnings diversified across businesses and geographies"). Multi-pass fallback regex picks up the first element title instead. Fix pending: add explicit `SLIDE_TITLE:` line as first entry in Step 4 pre-map. |
| **Decision** | Single-pass is more accurate for donut_dual_ring on this test (4/4 vs 3/4). Multi-pass still correct for text tables but introduces a degradation path for spatially encoded charts where the Step 4 pre-mapping can introduce errors that survive into Pass 2. See E-13 for the architectural routing design that follows from this finding. |
...
### E-13 — Pipeline Architecture Evolution: Multi-Pass → Hybrid Routing ● DESIGN / 2026-06-04

| | |
|---|---|
| **Context** | Three distinct pipeline architectures were built and compared across this project. The evolution is documented here for future reference. |
| **Architecture 1 — Original multi-pass (pre-redesign)** | Pass 1: `call_gemini([img_part, PASS1_PROMPT], text_only=True)` — image → plain text description. Pass 2: `call_gemini([img_part, p2_prompt])` — image + description → JSON DataPoints. Correction pass (if validation fails): `call_gemini([img_part, errors, description])` — image again. The image was sent on every call: 2× per slide normally, 3× when a correction fired. |
| **Architecture 2 — Redesigned multi-pass (current)** | Pass 1 unchanged: image → plain text with Step 4 pre-mapping. Pass 2 redesigned to **text-only**: `call_gemini([p2_prompt])` — no image, pure transcription of Step 4 into JSON. Correction pass also text-only. Image sent only once per slide. Motivation: reduce schema pressure during visual reading by separating the image call from the JSON call. Problem: the pre-mapping in Pass 1 can still drift (Consumer/PB swap in E-12), and Pass 2 has no image to self-correct against. |
| **Architecture 3 — Hybrid routing (proposed)** | Route by element type after Pass 0 classify. Visual slides (donut_dual_ring, waterfall, stacked_bar, stacked_bar_with_overlay, trend_line, kpi_grid, pie) → single-pass: `call_gemini([img_part, SINGLE_PASS_PROMPT])` — one call, image + JSON schema, no intermediate description. Text slides (text_table, npa_movement_table) → multi-pass unchanged: image → description → text-only JSON transcription. Image sent once for both paths. |
| **Comparison table** | See below. |
| **Why single-pass works better for visuals** | Single-pass keeps the image present while Gemini fills the schema. There is no intermediate text representation to drift. The model reasons about "what is the inner ring period?" and "what value goes in period=FY25?" simultaneously with the image in context. Multi-pass forces that reasoning to happen in Pass 1 text, then transcribe it in Pass 2 without the image — any error in the pre-map is frozen into the output. |
| **Why multi-pass still preferred for text tables** | Text tables benefit from the explicit Step 1–4 decomposition: inventory every number, establish column structure, verify arithmetic, then emit JSON. This structured decomposition catches sign errors and misread values before schema filling. A single-pass JSON prompt on a dense P&L table with 30 rows and 5 columns applies more schema pressure per call than a two-step describe-then-transcribe approach. |
| **Decision** | Implement hybrid routing in `process_slide()`: detect `is_visual` from `known_types` after Pass 0, branch to `extract_single_pass()` for visual slides or existing multi-pass for text slides. No change to validation, correction, audit saving, or renderer. |
| **Learning** | The right architecture depends on the content type. Visual encoding (spatial position of bars, ring order, colour) benefits from keeping the image present through schema filling. Textual encoding (P&L rows with printed numbers) benefits from structured decomposition that separates counting from structuring. A single architecture cannot optimise for both simultaneously at current model capability. |

**Pipeline comparison table:**

| | Original multi-pass | Redesigned multi-pass (current) | Hybrid routing (proposed) |
|---|---|---|---|
| Visual slides | Pass1(img) → Pass2(img+desc) | Pass1(img) → Pass2(text only) | Single call (img+schema) |
| Text slides | Pass1(img) → Pass2(img+desc) | Pass1(img) → Pass2(text only) | Pass1(img) → Pass2(text only) |
| Image sent (visual) | 2–3× | 1× | 1× |
| Image sent (text) | 2–3× | 1× | 1× |
| donut_dual_ring accuracy | Untested | 3/4 (Consumer/PB swapped) | 4/4 (E-12 single-pass result) |
| Root issue | Schema pressure during visual reading | Pre-map drift frozen in text | N/A — image present throughout |
